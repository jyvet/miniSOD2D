   ...
   use mod_constants
      ...
      contains
         subroutine full_convec_ijk_momentum(nelem,npoin,connec,He,gpvol,dlxigp_ip,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,u,q,rho,pr,Rmom)
            ...
            !$acc parallel loop tile(32) private(ipoin,idime,jdime,kdime,Re_mom,ul,ql,rhol,prl,fl,fuul) !!vector_length(vecLength) !!XTile
            do ielem = 1,nelem
               !$acc loop vector
               do inode = 1,nnode
               ...
               !$acc loop vector tile(16) private(dlxi_ip,dleta_ip,dlzeta_ip,gradIsoRho,gradIsoP,gradIsoU,gradIsoF,gradIsoFuu,gradIsoQ,gradRho,gradP,gradRE,gradU,divF,divU,divQ,gradQ,divFuu,isoI,isoJ,isoK,ii) !!YTile
               do igaus = 1,ngaus
                  !$acc loop seq
                  do ii=1,porder+1
         ...
         subroutine full_convec_ijk_energy(nelem,npoin,connec,He,gpvol,dlxigp_ip,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,u,q,rho,E,Rener)
            ...
            !$acc parallel loop tile(32) private(ipoin,idime,jdime,kdime,Re_ener,ul,ql,rhol,El,REl,fel,fuel,kl,Rkl,fkl,fukl) !!vector_length(vecLength) !!XTile
            do ielem = 1,nelem
               !$acc loop vector
               do inode = 1,nnode
                  ipoin(inode) = connec(ielem,inode)
               ...
               !$acc loop vector tile(16) private(dlxi_ip,dleta_ip,dlzeta_ip,gradIsoRho,gradIsoRE,gradIsoRk,gradIsoE,gradIsok,gradIsoU,gradIsoF,gradIsoFuu,gradIsoQ,gradIsoFe,gradIsoFue,gradIsoFk,gradIsoFuk,gradRho,gradE,gradRE,gradk,gradRk,gradU,divU,divQ,divFe,divFue,divFk,divFuk,gradQ,isoI,isoJ,isoK,ii) !!YTile
               do igaus = 1,ngaus
                  !$acc loop seq
                  do ii=1,porder+1
                     dlxi_ip(ii) = dlxigp_ip(igaus,1,ii)
         ...
         subroutine full_convec_ijk_mass(nelem,npoin,connec,He,gpvol,dlxigp_ip,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,u,q,rho,Rmass)
            ...
            !$acc parallel loop tile(32) private(ipoin,idime,jdime,kdime,Re_mass,ul,ql,rhol) !!vector_length(vecLength) !!XTile
            do ielem = 1,nelem
               ...
               !$acc loop vector tile(16) private(dlxi_ip,dleta_ip,dlzeta_ip,gradIsoRho,gradIsoU,gradIsoQ,gradRho,gradU,divU,divQ,gradQ,isoI,isoJ,isoK,ii) !!YTile
               do igaus = 1,ngaus
                  !$acc loop seq
                  do ii=1,porder+1
                     dlxi_ip(ii) = dlxigp_ip(igaus,1,ii)
